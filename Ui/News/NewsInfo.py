...
class NewsInfo(QWidget):
    ...
    def resizeEvent(self, a0: QResizeEvent) -> None:
        self.l_img.move(0, 0)
        self.l_img.resize(self.height(), self.height())
        self.l_img.resize(0, 0)

        self.l_title.move(self.l_img.width(), 0)
        font = self.l_title.font()
        fontm = QFontMetrics(font)
        self.l_title.resize(self.width(), fontm.height())

        self.l_description.move(self.l_img.width(), self.l_title.height())
        self.l_description.resize(
            self.width(), self.height()-self.l_title.height())
    ...
    def load_img(self):
        ...
        g.logapi.info(f"{url=}")
        r = requests.get(url)
        self.l_img.setPixmap(QPixmap.fromImage(QImage.fromData(r.content)))
